...
-- EXEC sp_RENAME 'Branch.BrnachAddress' , 'BranchAddress', 'COLUMN'


create table Borrower(
	CardNo INT not null identity(100,1),
...
-- EXEC sp_RENAME 'Borrower.BoroowerPhone' , 'BorrowerPhone', 'COLUMN'
create table BookCopies(
	CopiesID int not null identity(1,1),
	BookID int not null,
	BranchID int not null,
	No_Of_Copies int not null,
	constraint pk_copyid primary key(CopiesID),
	constraint fk_bookid foreign key(BookID) references book(ID),
	constraint fk_branchid foreign key(BranchID) references Branch(ID)
);
...
create table BookLoans(
	...
	BranchID int not null constraint fk_branch_id1 foreign key references Branch(ID) on update cascade on delete cascade,
...
create table Authors(
	...
	constraint fk_Bookid_Author foreign key(BookID) references book(ID)
);
...
INSERT INTO Publisher
	...
	SELECT * FROM book WHERE PublisherName = 'Bloomsbury'
	...
	SELECT bc.BranchID AS [Branch ID], br.BranchName AS [Branch Name],
	   bc.No_Of_Copies AS [Number of Copies],
	   bo.Title AS [Book Title]
	   FROM BookCopies AS bc
			INNER JOIN book AS bo ON bc.BookID = bo.ID
			INNER JOIN branch AS br ON bc.BranchID = br.ID
	   WHERE bo.Title = 'The Lost Tribe' AND br.BranchName = 'Sharpstown';
...
SELECT bc.BranchID AS [Branch ID], br.BranchName AS [Branch Name],
	   ...
	   FROM bookcopies AS bc
			INNER JOIN book AS bo ON bc.BookID = bo.ID
			INNER JOIN branch AS br ON BranchID = br.ID
	   WHERE bo.Title = 'The Lost Tribe' 
go

create proc dbo.loaners_info
(@DueDate date = Null, @BranchName varchar(50) = "Sharpstown")
as
select br.BranchName AS [Branch Name],  Bo.Title [Book Name],
	   Bor.BorrowerName AS [Borrower Name], Bor.BorrowerAddress AS [Borrower Address],
	   L.DateOut AS [Date Out], L.DueDate [Due Date]
	   FROM BookLoans AS l
			INNER JOIN book AS bo ON L.BookID = bo.ID
			INNER JOIN borrower AS bor ON L.CardNo = bor.CardNo
			INNER JOIN branch AS Br ON l.BranchID = br.ID
		WHERE l.DueDate = @DueDate AND br.BranchName = @BranchName
GO
EXEC dbo.loaners_Info 
